...
**最後更新：** 2026-08-25
**目前位置：** 分支 `main`。**19/19 全部完成，Plan 1 的程式與文件都到位。**

**尚未完成的是「人工驗收」，不是程式**——見〈剩下要人做的事〉。
Plan 1 的完成標準有六條，其中兩條（離線可用、iPhone 主畫面啟動）
只能在實機上確認。
...
### Phase 4：上線
- [x] 4.1 階段 1 的個別元素圖像掛鉤 — Opus（`a805930`）
- [x] 4.2 部署與說明文件 — Opus（`840c174`；部署已於 Phase 3 提前完成）
- [+] 4.1b 八族場景插圖 — Opus（`acacbb8` 自繪 SVG、`c1e5fe8` 使用者提供的手繪風全圖）

---

## 剩下要人做的事

程式已全部完成。以下三件只能由人做，做完 Plan 1 才算真正結束：

1. **Service Worker 更新驗證（Task 4.2 Step 3）**
   開 https://mcdull419-tw.github.io/periodic-quest/ ，**不要清任何快取**，
   直接重整。首頁最下方若出現「版本 2026-08-25」就代表更新路徑是通的。
   沒出現的話，SW 或 HTTP 快取還有問題，診斷方法見上面〈踩過的坑〉。
   （伺服器端已確認：Pages 上的 `screen-home.js` 已含 `build-mark`。）

2. **iPhone 實機驗收（Task 4.2 Step 4）**
   Safari 開網址 → 分享 → 加入主畫面 → 從主畫面啟動。確認：
   沒有 Safari 網址列、**開飛航模式仍能開啟並作答**、關掉再開進度還在。

3. **讓學生試用 1A**（spec §11.4）
   七組圖像掛鉤與八族插圖的風格是否合用。確認後才派 sub-agent 依關卡
   分批補其餘六關——這是「先驗證再量產」的驗證步驟，不可跳過。

## 下一步（程式面）

Plan 1 已完成，接下來是撰寫 **Plan 2**：遊戲層（連段、應援值、賽末點）、
其餘六關的圖像掛鉤內容、設定畫面與自訂圖片、圖鑑收藏。

派工原則（使用者 2026-08-24 指示）：**以 token 使用量為優先判準**。
圖像掛鉤那類「格式固定、彼此獨立、脈絡淺」的內容生產派給 sub-agent；
依賴大量既有約定的實作自己做。
...
`zhuyin` 6、`mnemonics` 10、`scene-art` 4。合計 **139**（帶 fixture 全串起來跑的數字）。
